...
// formats are supported, trading off color fidelity for  space.
...
uint8_t  imageNumber   = 0,  // Current image being displayed
        ...
        *imagePalette,       // -> palette data in 
        *imagePixels,        // -> pixel data in 
...
const uint16_t  lineTable[] = { // 375 * 2^(n/3)
...
void loop() {
  ...
  switch(imageType) {
    ...
    case PALETTE8: { // 8-bit (256 color) -palette-based image
      uint16_t  o;
      uint8_t   pixelNum,
